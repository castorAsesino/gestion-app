...
export default function EvaluarCapacidadAtributoForm(props) {
  ...
  const handleChange = (event, id) => {
    ...
    const updatedRows = rows.map((row) => {
      if (row.id === id) {
        const selectedcalificacion = calificaciones.find(
          (calificacion) => calificacion.id === value
        );
        return {
          ...row,
          calificacion: value,
          nivel: selectedcalificacion ? selectedcalificacion.nombre : "",
          valor: selectedcalificacion ? selectedcalificacion.valor : "",
        };
      }
      return row;
    });
  ...
  useEffect(() => {
    const allCalificacionesAsignadas = rows.every(
      (row) => row.calificacion > 0
    ); // Verifica que todas las calificaciones sean mayores a 0
  ...
  const calcularTotal = (id) => {
    ...
    setRows((prevRows) =>
      prevRows.map((row) =>
        row.id === id
          ? { ...row, totalPonderacion: row.calificacion * row.ponderacion }
          : row
      )
    );
    console.log(rows);
  ...
  const validar = () => {
    ...
    const valorMasAlto = calificaciones.reduce((max, valor) =>
      max.valor > valor.valor ? max : valor
    );
    ...
    <Container component="main" className={classes.main}>
      ...
      <TableContainer component={Paper}>
        <Table className={classes.table} aria-label="customized table">
          <TableHead>
            <TableRow>
              <StyledTableCell className={classes.headerStyle}>
                Atributo de Proceso
              </StyledTableCell>
              <StyledTableCell className={classes.headerStyle} align="center">
                Descripción
              </StyledTableCell>
              <StyledTableCell className={classes.headerStyle} align="center">
                Calificación de Logro
              </StyledTableCell>
      ...
      <div>
        ...
        >
          ...
          <DialogContent dividers>
            ...
            >
              <li
                style={{ fontSize: 18, fontWeight: 800, marginRight: "1rem" }}
              >
            ...
            <TableContainer component={Paper}>
              ...
              >
                <TableHead>
                  <TableRow>
                    <TableCell
                      style={{ backgroundColor: "#4576e0", color: "#fff" }}
                    >
                      Proyecto
                    </TableCell>
                    <TableCell
